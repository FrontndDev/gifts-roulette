...
export const TabsContent = ({
  ...
  return (
    <div className="relative flex flex-col gap-[37px] pt-[27px]">
      <Icons.Close
        className="absolute top-0 right-0 h-[19px] w-[19px] cursor-pointer"
        onClick={() => setShowTabs(false)}
      />
      <div className="m-auto flex h-[37px] w-[75.12vw] items-center rounded-[1583px] bg-[#171717]">
        {tabs.map((tab, i) => (
          <div
            key={tab.value}
            className={cn(
              'flex h-full w-full cursor-pointer items-center justify-center rounded-[959px] text-center text-[11px] leading-[9px] font-[500] text-[#BABABA]',
